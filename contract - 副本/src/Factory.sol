...
contract Factory {
    ...
    constructor(
        ...
        require(msg.sender != entryEOA, "deployer can't be entry");
        deployer = msg.sender;
    ...
    function _newAccount(
        ...
        AccountEntity(payable(acct)).initUpgradeImpl(
            accountImpl,
            _getBigBrotherAccount(_ownerId)
        );

        bytes memory data_initByFactory = abi.encodeWithSignature(
            "initByFactory(address,address,string,uint256)",
            entryEOA,
            _getPasswdAddr(_ownerId, _passwdAddr),
            _getQuestionNos(_ownerId, _questionNos),
            _depositFeeAmount // gasFreeAmount
        );
        (bool ok, ) = acct.call(data_initByFactory);
        require(ok, "call initByFactory failed");
