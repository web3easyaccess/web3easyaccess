...
export function PrivateInfo({
    ...
    console.log(
        `privateinfo..123,forTransaction=${forTransaction},bigBrotherAccountCreated(userProp)=${bigBrotherAccountCreated(
            userProp
        )}`
    );
    ...
    const fetchBigBrothersQuestionNos = async (oldPin: string) => {
        if (oldPin != "") {
            ...
            if (forModification()) {
                ...
                setMyOldFirstQuestionNo(no1);
                setMyOldSecondQuestionNo(no2);
            } else {
                currentPriInfoRef.current.firstQuestionNo = no1;
                currentPriInfoRef.current.secondQuestionNo = no2;
                setMyFirstQuestionNo(no1);
    ...
    const handlePinBlur = (position: string) => {
        console.log(
            "handlePinBlur,ac:",
        ...
        if (position == "pin1") {
            if (pin1 == null || pin1.length == 0 || !pwdRegex.test(pin1)) {
                setPinErrorMsg(
        ...
        if (!bigBrotherAccountCreated(userProp)) {
            ...
            if (forTransaction) {
                ...
                if (pin_old != null && pin_old.length > 0) {
    ...
    const handlePin1Blur = () => {
        handlePinBlur("pin1");
    };
    const handlePin2Blur = () => {
        handlePinBlur("pin2");
    };
    const handlePinOldBlur = () => {
        handlePinBlur("pinOld");
    };
    ...
    const [myOldFirstQuestionNo, setMyOldFirstQuestionNo] = useState(
        oldPriInfoRef.current.firstQuestionNo
    );
    const [myOldSecondQuestionNo, setMyOldSecondQuestionNo] = useState(
        oldPriInfoRef.current.secondQuestionNo
    );

    const onSelectionChange1Old = (key: string) => {
        oldPriInfoRef.current = {
            ...currentPriInfoRef.current,
            firstQuestionNo: key,
        };
        setMyOldFirstQuestionNo(key);
    };
    const onSelectionChange2Old = (key: string) => {
        oldPriInfoRef.current = {
            ...currentPriInfoRef.current,
            secondQuestionNo: key,
        };
        setMyOldSecondQuestionNo(key);
    };
        ...
        <>
            ...
            >
                <CardHeader className="flex gap-3">
                    ...
                    <Switch
                        isSelected={privateinfoHidden}
                        onValueChange={updatePrivateinfoHidden}
                        style={{ marginLeft: "150px" }}
                        defaultSelected={privateinfoHidden}
                        size="sm"
                        color="warning"
                    >
                        Hidden
                                ...
                                {forModification() ? (
                                    <div style={{ marginBottom: "10px" }}>
                                        <Autocomplete
                                            ...
                                            selectedKey={myOldFirstQuestionNo}
                                            defaultSelectedKey={
                                ...
                                {forModification() ? (
                                    <div style={{ marginBottom: "10px" }}>
                                        <Autocomplete
                                            ...
                                            selectedKey={myOldSecondQuestionNo}
                                            defaultSelectedKey={
    ...
    const myMsg = () => {
        if (forTransaction) {
            if (accountAddrCreated(userProp)) {
                // account has created.
                return {
                    msg: `Sender: ${readAccountAddr(userProp)}.`,
                    color: "success",
                };
            } else {
                if (bigBrotherAccountCreated) {
                    return {
                        // No account has been created under your email [${userProp.email}]
                        msg: `Sender: ${readAccountAddr(
                            userProp
